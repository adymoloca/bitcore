...
We're working hard to make *flocore* the most powerful JavaScript library for working with flocoin. Our goal is to have *flocore* be a library that can be used by anyone interested in flocoin, and to level expertise differences with great design and documentation.
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Flocore methods. For example, calling `JSON.stringify` on a Flocore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
```sh
git remote add flocoin-project git@github.com:flocoin-project/flocore.git
git pull --rebase flocoin-project master
```
...
```
Finally go to [github.com/flocoin-project/flocore](https://github.com/flocoin-project/flocore) in your web browser and issue a new pull request.
...
[github.com/flocoin-project/flocore/issues](https://github.com/flocoin-project/flocore/issues).
